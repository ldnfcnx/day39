...
function setUrl(){
  ...
  if(window.location.href.indexOf("?r")!=-1){
     str=window.location.href.slice(0,window.location.href.indexOf("?r"));
  }else{
     str= window.location.href;
  }
    str+="?r="+regionArr.toString()+"?r"+"?p="+productArr.toString()+"?p";
  window.history.pushState({}, null, str);
}
function parseUrl(){
 let arr= [];
     str=decodeURIComponent(window.location.href);   //字符转码（火狐浏览器会将中文URL转化为UTF-8字符，需转化）
 arr[0]=str.match(/\?r=(\S*)\?r/)[1];    //正则表达式匹配两指定字符之间
 arr[1]=str.match(/\?p=(\S*)\?p/)[1]; 
 return arr;
}
function urlCheck(){
  let str=parseUrl().join();
